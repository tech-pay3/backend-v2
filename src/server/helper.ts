...
export async function handleUserRegResponse(
  response: Response,
  context: Context,
  vaultService: VaultService // Replace with actual VaultService type
): Promise<void> {
  ...
  try {
    ...
    throw new HTTPException(400, { message: 'Failed to process user registration' })
  }
}
...
export const userRegContextMiddleware = async (c: Context, next: () => Promise<void>) => {
