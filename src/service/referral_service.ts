...
export class ReferralService {
  ...
  async useReferralCode(
    ...
    // Find the referee by their external ID
    const { data: referee, error: refereeError } = await this.supabase
      .from("users")
      .select("id")
    ...
    const { data, error } = await this.supabase.from("user_referrals").insert({
      referrer_id: referrer.external_id,
      referee_id: userExternalId,
    });
  ...
  async completeQuest(
    ...
    const completed = await this.questManager.isQuestCompleted(
      quest,
      externalId
    );
